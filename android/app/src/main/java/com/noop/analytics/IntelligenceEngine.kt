 ...
 *     covered.
 *
 * This is what makes Choop independent of WHOOP's cloud , for any day the strap
 * collected raw data with Choop connected, Choop scores it itself rather than relying on
...
object IntelligenceEngine {
    ...
    /** Imported wearable-export source ids whose DAILY aggregates can be scored for a Choop Charge/Rest on
    ...
    private suspend fun analyzeRecentOnCpu(
        ...
        for (offset in 0 until maxDays) {
            ...
            // things BY DESIGN, not a bug: Choop's restingHr is the WHOOP-style FLOOR (the lowest
            ...
            // over the whole asleep span. The mean always sits above the floor, so Choop looking lower is
        ...
        // untouched; the computed row carries the Choop-derived Charge + the Rest composite. HONEST DATA: the
    ...
    /**
     * The per-day RHR floor-vs-mean diagnostic line (#691). Choop's [floor] is the WHOOP-style resting
     ...
     * over the whole asleep span. The mean always sits at-or-above the floor, so Choop reading lower is
     * BY DESIGN, not a bug; logging both makes a "Choop RHR is lower than my other app" report explainable
